...
$('#contentCard1').one('launch', function () { //launch once
    ...
    //document.getElementById('sliderKnob').addEventListener('touchstart', function(e) {
        startPos = e.pageY;
        pressed = true;
        $('#cardTitle').html = startPos;
    },
    //$('#cardFrame').mousemove(function (e) {
    //$('#cardFrame').on('touchmove mousemove', function (e) {
    //document.getElementById('cardFrame').addEventListener('touchmove', function(e) {
    'touchmove mousemove': function (e) {
        dragPos = e.pageY;
        heightInCm = Math.round(((fillH + (startPos - dragPos)) * imgH) / refH);

        //Touchscreen compatibility, TODO: use position absolute, bottom: x px; introduce tap/click functionality on
        //reference image to reposition slider... i.e. slide anywhere
        if (pressed && heightInCm > 0 && heightInCm <= imgH) {
            $('#sliderKnob').css({
                'transform': 'translateY(' + (translateVar + dragPos - startPos) + 'px)'
            });
